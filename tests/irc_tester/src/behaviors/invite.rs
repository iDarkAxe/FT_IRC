...
pub async fn invite_not_on_channel(port: u16, id: usize, timeout_ms: u64) -> Result<()> {
    ...
    let mut client = Client::connect(port).await?;
    client.authenticate(nick, timeout_ms).await?;
    client.try_expect("INVITE reserved_nick #channel\r\n", " :You're not on that channel", "ERR_NOTONCHANNEL missing", timeout_ms).await?;
    client.shutdown().await?;
    Ok(())
}
...
pub async fn invite_no_such_nick(port: u16, id: usize, timeout_ms: u64) -> Result<()> {
    ...
    let mut client = Client::connect(port).await?;
    client.authenticate(nick, timeout_ms).await?;
    client.try_expect("JOIN #join_no_such_nick_test\r\n", "JOIN #join_no_such_nick_test", "Failed to join a valid chan", timeout_ms).await?;
    client.try_expect("INVITE nosuchnickname #join_no_such_nick_test\r\n", " :No such nick/channel", "Err_NOSUCHNICK missing", timeout_ms).await?;
    client.shutdown().await?;
    Ok(())
}
...
pub async fn invite_no_priv(port: u16, id: usize, timeout_ms: u64) -> Result<()> {
    ...
    let mut client = Client::connect(port).await?;
    client.authenticate(nick, timeout_ms).await?;
    client.try_expect("JOIN #join_no_priv\r\n", "JOIN #join_no_priv", "Failed to join a valid chan", timeout_ms).await?;
    client.try_expect("INVITE reserved_nick #join_no_priv\r\n", " :You're not channel operator", "ERR_CHANOPRIVSNEED missing", timeout_ms).await?;
    client.shutdown().await?;
    Ok(())
}
