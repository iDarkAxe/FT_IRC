...
pub async fn topic_not_on_chan(port: u16, id: usize, timeout_ms: u64) -> Result<()> {
    ...
    client.authenticate(nick, timeout_ms).await?;
    client.try_expect("TOPIC #no_mdp_chan\r\n", " :You're not on that channel", "ERR_NOTONCHAN missing ", timeout_ms).await?;
...
pub async fn topic_no_topic(port: u16, id: usize, timeout_ms: u64) -> Result<()> {
    ...
    client.authenticate(nick, timeout_ms).await?;
    client.try_expect("JOIN #no_mdp_chan\r\n", "JOIN #no_mdp_chan", "Failed to join #no_mdp_chan ", timeout_ms).await?;
    client.try_expect("TOPIC #no_mdp_chan\r\n", " :No topic is set", "ERR_NOTOPIC missing ", timeout_ms).await?;
    client.shutdown().await?;
