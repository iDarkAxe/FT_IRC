...
async fn main() -> Result<()> {
    ...
    //faux mot passe
    normal_connection_wrong_password(port).await?;
    ...
    fragemented_messages(port).await?; //Après X secondes sans \r\n → timeout
    low_bandwidth(port).await?;

    //client qui se connecte partiellement, qui repond au ping, mais qui reste la : timeout de 10s
    uncomplete_inactive_connection(port).await?;
...
async fn uncomplete_inactive_connection(port: u16) -> Result<()> {
...
async fn normal_connection_wrong_password(port: u16) -> Result<()> {
    let stream = TcpStream::connect(("127.0.0.1", port)).await?;
    let (reader, mut writer) = stream.into_split();
    let mut reader = BufReader::new(reader);
    let mut line = String::new();
...
async fn low_bandwidth(port: u16) -> Result<()> {
    let stream = TcpStream::connect(("127.0.0.1", port)).await?;
    let (reader, mut writer) = stream.into_split();
    let mut reader = BufReader::new(reader);
    let mut line = String::new();
